...
export const recipeService = {
  ...
  generateRecipe: async (ingredients: string[], precise: boolean = false): Promise<Recipe> => {
    if (!import.meta.env.VITE_GEMINI_API_KEY) {
      throw new Error("AI Kitchen Lab is currently offline (Missing API Key). Please try again later.");
    }
...
Ensure the cooking instructions are clear and professional. Return the recipe in JSON format.`;
    ...
    let recipeData;
    try {
      // Use the recommended Gemini 3 model for text tasks
      ...
      if (apiError.message?.includes("400") || apiError.message?.includes("404")) {
        throw new Error("The AI Laboratory is updating its knowledge. Please try again with different ingredients in a moment.");
      }
